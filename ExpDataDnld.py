...
HMGULA_params_df = whole_df[whole_df['pipelineKey'].isin(['HMGULA_001', 'HMGULA_002'])]
HMGULA_params_df = HMGULA_params_df[HMGULA_params_df['type'].isin(['simpleParameter', 'seriesParameter', 'ontologyParameter'])]
HMGULA_params_keys = HMGULA_params_df['parameterKey']
HMGULA_params_keys = HMGULA_params_keys.tolist()
HMGULA_params_keys = list(set(HMGULA_params_keys))
...
with ThreadPoolExecutor(max_workers=max_workers) as executor:
    ...
    futures = {executor.submit(download_data, url): url for url in urlsExp_LA_25k}
    try:
...
print(f"There are {len(paramsDataExp_LA)} URL's")
